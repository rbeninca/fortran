...
window.onload = () => {
  ...
  chart = new Chart(ctx, {
    type: 'line',
    data: { labels: [], datasets: [{ label: 'Força', data: [], borderColor: 'var(--cor-primaria)', backgroundColor: 'rgba(59, 130, 246, 0.1)', borderWidth: 2, fill: true, tension: 0.4 }] },
    options: { 
      animation: false, responsive: true, 
      scales: { x: { title: { display: true, text: 'Tempo (s)' } }, y: { title: { display: true, text: 'Força (N)' } } },
...
function updateUI(dado) {
  ...
  if (rawDataN.length > MAX_DATA_POINTS && chartMode === 'deslizante') {
    rawDataN.shift(); labels.shift(); datasets[0].data.shift();
  }
  ...
  let massaKg = 0;
  if (gravity && gravity > 0) { massaKg = forceN / gravity; }
...
function sendCommand(cmd) { if (socket && socket.readyState === WebSocket.OPEN) { socket.send(cmd); } else { showNotification("error", "Não conectado. Impossível enviar comando."); } }
...
function setChartMode(mode) {
    chartMode = mode;
    document.querySelectorAll('.flex-center .btn-grupo .btn').forEach(b => {
...
function clearChart() {
    chart.data.labels = []; chart.data.datasets[0].data = []; rawDataN = [];
...
function resetChartZoom() { chart.resetZoom(); }
...
function showNotification(type, message, duration = 7000) {
    ...
    if(duration > 0) setTimeout(() => { notification.style.transition = 'opacity 0.5s'; notification.style.opacity = '0'; setTimeout(() => notification.remove(), 500); }, duration);
}
...
function salvarGravacao() {
    ...
    const gravacao = { id: Date.now(), nome: nome, timestamp: new Date().toISOString(), dadosTabela: dadosTabela.reverse(), imagemGrafico: chart.toBase64Image() };
    try {
    ...
    gravacoes.reverse().forEach(gravacao => {
        ...
        card.innerHTML = `
            ...
            <div class="botoes">
                ...
                <button onclick="salvarImagemGrafico(${gravacao.id})" class="btn btn-primario">Salvar Gráfico</button>
...
function salvarImagemGrafico(id) {
    ...
    link.setAttribute('download', `grafico_balanca_${gravacao.nome.replace(/ /g,"_")}_${id}.png`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
}
